...
export default function RotacionPage() {
  ...
  const salEmpCompTraces = anosDisponibles.map((ano) => ({
    ...
    x: empresasComp,
    y: empresasComp.map((emp) => compData[ano]?.salEmp.find((r: AnyObj) => (r.EMPRESA ?? r.empresa) === emp)?.salidas ?? 0),
            ...
            {motOrig.length > 0 && (
              <ChartCard title="Motivos de Salida">
                <PlotChart
                  ...
                  layout={{
                    margin: { t: 10, r: 50, b: 30, l: 10 },
                    xaxis: { range: [0, Math.max(...motOrig.map((r) => r.cantidad)) * 1.15] },
              ...
              {motivoEmpTraces.length > 0 && (() => {
                return (
                  <>
                    ...
                    <PlotChart
                      light
                      data={motivoEmpTraces.map((t) => ({
                        ...t,
                        text: t.y.map((v: number) => v > 0 ? String(v) : ""),
                        textposition: "outside",
                      })) as AnyObj[]}
                      layout={{
                        barmode: "group",
                        margin: { t: 20, r: 16, b: 80, l: 50 },
              ...
              {nivelesEmp.emps.length > 0 && nivelesEmp.niveles.length > 0 && (
                <ChartCard title="Salidas por Nivel AIC y Empresa">
                  <PlotChart
                    ...
                    height={Math.max(320, nivelesEmp.emps.length * 110)}
                  />
                </ChartCard>
              )}
            </>
        ...
        <ChartCard title="Mapa de satisfacción: empresa × dimensión">
          <PlotChart
            ...
            data={[{
              ...
              y: dimsUniq,
              z: heatZ,
              colorscale: [[0, "#b30000"], [0.6, "#fc8d59"], [1, "#f9c4a0"]],
              zmin: 1,
              zmax: 5,
